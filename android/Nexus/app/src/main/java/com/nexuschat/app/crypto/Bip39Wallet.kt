...
class Bip39Wallet private constructor(
    private val mnemonic: List<String>,
    private val seed: ByteArray,
    private val masterKey: ByteArray
) {
    companion object {
        private const val TAG = "NexusChat/BIP39"
        ...
        @Volatile private var instance: Bip39Wallet? = null

        fun generate(wordCount: Int = 12, passphrase: String = ""): Bip39Wallet {
        ...
        fun fromMnemonic(mnemonicWords: String, passphrase: String = ""): Bip39Wallet? {
            val words = mnemonicWords.trim().lowercase().split(Regex("\\s+"))
            if (words.size != 12 && words.size != 24) {
                Log.e(TAG, "Invalid mnemonic length: ${words.size}")
                return null
            }
            val indices = words.map { w ->
                val idx = WORDLIST.indexOf(w)
                if (idx < 0) { Log.e(TAG, "Unknown word: $w"); return null }
                idx
            }
            val entropy = recoverEntropy(indices)
            if (entropy == null) { Log.e(TAG, "Checksum mismatch"); return null }
            return fromEntropy(entropy, passphrase)
        }

        fun fromEntropy(entropy: ByteArray, passphrase: String = ""): Bip39Wallet {
            val checksum = sha256(entropy)
            val totalBits = entropy.size * 8
            val cs = totalBits / 32
            val bits = mutableListOf<Int>()
            entropy.forEach { b -> for (i in 7 downTo 0) bits.add((b.toInt() shr i) and 1) }
            for (i in 0 until cs) bits.add((checksum[i / 8].toInt() shr (7 - (i % 8))) and 1)
            val indices = bits.chunked(11).map { chunk -> chunk.fold(0) { acc, b -> (acc shl 1) or b } }
            val words = indices.map { WORDLIST[it] }
            val mnemonic = words.joinToString(" ")
            val seed = generateSeed(mnemonic, passphrase)
            val masterKey = deriveBip32MasterKey(seed)
            Log.i(TAG, "BIP39 wallet: ${words.size} words, seed=${seed.size}B")
            return Bip39Wallet(words, seed, masterKey).also { instance = it }
        }
        ...
        private fun recoverEntropy(indices: List<Int>): ByteArray? {
            val totalBits = indices.size * 11
            val entropyBits = totalBits - totalBits / 33
            val bits = mutableListOf<Int>()
            indices.forEach { idx -> for (i in 10 downTo 0) bits.add((idx shr i) and 1) }
            val entropyBytes = entropyBits / 8
            val entropy = ByteArray(entropyBytes)
            for (i in 0 until entropyBits) {
                if (bits[i] == 1) entropy[i / 8] = (entropy[i / 8].toInt() or (1 shl (7 - (i % 8)))).toByte()
            }
            val checksumBits = totalBits - entropyBits
            val checksum = sha256(entropy)
            for (i in 0 until checksumBits) {
                val expected = (checksum[i / 8].toInt() shr (7 - (i % 8))) and 1
        ...
        fun generateSeed(mnemonic: String, passphrase: String): ByteArray {
            ...
            val params = gen.generateDerivedParameters(512)
            return (params as KeyParameter).key
        }

        private fun deriveBip32MasterKey(seed: ByteArray): ByteArray {
            val hmac = HMac(SHA512Digest())
            hmac.init(KeyParameter("Bitcoin seed".toByteArray(Charsets.UTF_8)))
    ...
    fun getMnemonic(): String = mnemonic.joinToString(" ")
    fun getWords(): List<String> = mnemonic.toList()
    fun getSeed(): ByteArray = seed
    fun getMasterKey(): ByteArray = masterKey
    fun getSeedB64(): String = Base64.encodeToString(seed, Base64.NO_WRAP)
    fun getMasterKeyB64(): String = Base64.encodeToString(masterKey, Base64.NO_WRAP)

    fun deriveBip44Key(path: String): ByteArray {
        val segments = path.removePrefix("m/").split("/").map { s ->
            if (s.endsWith("'")) (s.dropLast(1).toInt() or 0x80000000.toInt()) else s.toInt()
        }
        var cur = masterKey
        for (seg in segments) cur = ckdPriv(cur, seg)
        return cur.copyOfRange(0, 32)
    }

    fun deriveBtcKey(): ByteArray = deriveBip44Key("m/44'/0'/0'/0/0")
    fun deriveEthKey(): ByteArray = deriveBip44Key("m/44'/60'/0'/0/0")

    fun deriveEd25519Keypair(seedData: ByteArray = seed): Pair<ByteArray, ByteArray> {
        val hmac = HMac(SHA512Digest())
        hmac.init(KeyParameter("ed25519 seed".toByteArray(Charsets.UTF_8)))
        val master = ByteArray(64)
        hmac.update(seedData, 0, seedData.size)
        hmac.doFinal(master, 0)
        var cur = master
        val path = listOf(44 or 0x80000000.toInt(), 0 or 0x80000000.toInt(),
            0 or 0x80000000.toInt(), 0 or 0x80000000.toInt(), 0 or 0x80000000.toInt())
        for (seg in path) {
            val idx = ByteArray(4).apply {
                this[0] = (seg shr 24).toByte()
                this[1] = (seg shr 16).toByte()
                this[2] = (seg shr 8).toByte()
                this[3] = seg.toByte()
            }
            val data = byteArrayOf(0x00.toByte()) + cur.copyOfRange(0, 32) + idx
            hmac.init(KeyParameter(cur.copyOfRange(32, 64)))
            cur = ByteArray(64)
            hmac.update(data, 0, data.size)
            hmac.doFinal(cur, 0)
        }
        val privKey = cur.copyOfRange(0, 32)
        val pubKey = deriveEd25519Public(privKey)
        return Pair(privKey, pubKey)
    }

    private fun deriveEd25519Public(privateKey: ByteArray): ByteArray {
        val keyParams = org.bouncycastle.crypto.params.Ed25519PrivateKeyParameters(privateKey, 0)
        return keyParams.generatePublicKey().encoded
    }

    fun deriveX25519Keypair(privateKeyBytes: ByteArray): Pair<ByteArray, ByteArray> {
    ...
    private fun ckdPriv(keyAndChain: ByteArray, index: Int): ByteArray {
        val key = keyAndChain.copyOfRange(0, 32)
        val chainCode = keyAndChain.copyOfRange(32, 64)
        val idxBytes = ByteArray(4).apply {
            this[0] = (index shr 24).toByte()
            this[1] = (index shr 16).toByte()
            this[2] = (index shr 8).toByte()
            this[3] = index.toByte()
        }
        val data = byteArrayOf(0x00) + key + idxBytes
        return hmacSha512(chainCode, data)
    }

    fun signData(data: ByteArray): ByteArray {
        val (priv, _) = deriveEd25519Keypair(seed)
        val signer = Ed25519Signer()
        signer.init(true, org.bouncycastle.crypto.params.Ed25519PrivateKeyParameters(priv, 0))
    ...
    override fun toString(): String = "Bip39Wallet(${mnemonic.size} words)"
}
